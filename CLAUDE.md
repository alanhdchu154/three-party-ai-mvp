...
- Use Alan's current two-loop flow: Central Umi / Codex defines direction, acceptance criteria, and source-of-truth context; cc does deep VS Code Claude Code / code-mode work for complex corpus, privacy, benchmark, or implementation tasks; Codex reviews, verifies, accepts/rejects/narrows, then updates the right source-of-truth files. For broad or risky changes, ask cc for an independent findings-first review of current diff/status before treating the work as complete.
