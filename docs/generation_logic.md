...
但 `occurred_at` 不應該永遠是「最近 24-72 小時」。Saga A 需要有時間厚度：同一個角色在國中後段、高一、高二、高三、申請季、轉學前後，對 AI 的使用方式和壓力來源都不同。生成前要先選一個 timeline slice：
...
如果是 `live_event`，`occurred_at` 可以落在該事件發生當天。如果是 `old_memory` 或 `pattern_reflection`，`occurred_at` 代表「角色跟 AI 談這件事的時間」，而 `scenario_seed` 必須明確說這件事本身發生在更早的國中 / 高一 / 高二 / 高三某段時間。
